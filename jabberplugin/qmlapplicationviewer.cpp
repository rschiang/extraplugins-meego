...
void QmlApplicationViewer::setMainQmlFile(const QString &file)
{
    ...
    d->view->rootContext()->setContextProperty("serverset", QVariant(true));
                                       ...
                                       QVariant aenabled)
{
    ...
    if (!uname.contains("@"))
        uname = uname + "@jabber.org";

    parameters["account"]=uname;
    parameters["password"]=pword;
    if (port.length()==0)
    {
        parameters["port"]=(uint)5222;
    }
    else
        parameters["port"]=port.toUInt();
    if (server.length()==0)
    {
        parameters["server"]=uname.split("@")[1];
    }
    else
    ...
    connect(mAcc->updateParameters(parameters, QStringList()),
            SIGNAL(finished(Tp::PendingOperation *)),
                                         ...
                                         QVariant dicon)
{
    if (!uname.toString().contains("@"))
        uname = QVariant(uname.toString() + "@jabber.org");
