...
void QmlApplicationViewer::setMainQmlFile(const QString &file)
{
    ...
    d->view->rootContext()->setContextProperty("encryptionset", QVariant(true));
    d->view->rootContext()->setContextProperty("extratextvisible", QVariant(true));
    ...
    if (initType==1 && accountx!=NULL)
    {
        ...
        for (int i=0; i<mAccSet->accounts().count(); i++)
        {
            if (mAccSet->accounts()[i]->uniqueIdentifier() == uid)
            {
                ...
                d->view->rootContext()->setContextProperty("usernametext", parameters.value("account","").toString().split("@")[0]);
                                       ...
                                       QVariant aenabled)
{
    ...
    uname = uname + "@odnoklassniki.ru";

    parameters["account"]=uname;
    parameters["password"]=pword;
    if (port.length()==0)
    {
        parameters["port"]=(uint)443;
    }
    else
        parameters["port"]=port.toUInt();

    parameters["require-encryption"]=encryption;
...
void QmlApplicationViewer::createAccount(QVariant dname,
                                         ...
                                         QVariant dicon)
{
    uname = QVariant(uname.toString() + "@odnoklassniki.ru");
...
void QmlApplicationViewer::onAccountxCreated(Accounts::AccountId id)
{
    ...
    connect(mAM->createAccount(QString("gabble"),
                               QString("jabber"),
